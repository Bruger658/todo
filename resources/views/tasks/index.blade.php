                    ...
                    <section class="mt-8 rounded-3xl border border-white/10 bg-slate-950/50 p-5 shadow-xl shadow-slate-950/20">
                     ...
                     @foreach ($tasks as $task)
                        ...
                        <div
                            ...
                            data-task-card
                        >
                            <article class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border p-6 shadow-2xl shadow-slate-950/40 {{ $isOverdue ? 'border-rose-400/40 bg-rose-950' : 'border-white/10 bg-slate-900' }}">
                                <div class="flex items-start justify-between gap-4">
                                    <div>
                                        <p class="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-300">Actividad</p>
                                        <h3 class="mt-2 text-2xl font-bold text-white {{ $task->isCompleted() ? 'line-through opacity-70' : '' }}">{{ $task->title }}</h3>
                                    </div>
                                    <button type="button" class="rounded-full border border-white/10 px-3 py-1 text-sm font-semibold text-slate-200 hover:bg-white/10" data-task-card-close>Cerrar</button>
                                </div>
                                ...
                                <div class="mt-5 grid gap-3 text-sm text-slate-300 sm:grid-cols-2">
                                    ...
                                    @endif
                                    @if ($isOverdue)
                                        <p class="font-bold text-rose-200">Pendiente</p>
                                    @endif
                                </div>

                                @if ($task->description)
                                    <p class="mt-4 rounded-2xl bg-white/5 p-4 text-sm text-slate-300">{{ $task->description }}</p>
                                @endif
                                ...
                                <div class="mt-6 flex flex-wrap gap-3">
                                    @if ($task->isCompleted())
                                        <form method="POST" action="{{ route('tasks.toggle', $task) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button class="rounded-2xl border border-white/10 px-4 py-2 text-sm font-semibold text-amber-200 hover:bg-amber-300/10">Reabrir</button>
                                        </form>
                                    @else
                                        <button type="button" class="rounded-2xl border border-white/10 px-4 py-2 text-sm font-semibold text-emerald-200 hover:bg-emerald-300/10" data-completion-choice-open="completion-choice-{{ $task->id }}">Hecha</button>
                                    @endif
                                    <form method="POST" action="{{ route('tasks.destroy', $task) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button class="rounded-2xl border border-rose-300/30 px-4 py-2 text-sm font-semibold text-rose-200 hover:bg-rose-300/10">Eliminar actividad</button>
                                    </form>
                                </div>
                                    ...
                                    <form method="POST" action="{{ route('tasks.update', $task) }}" class="mt-4 grid gap-3 sm:grid-cols-2">
                                        @csrf
                                        @method('PUT')
                                        <input name="title" value="{{ old('title', $task->title) }}" required class="rounded-xl bg-white px-3 py-2 text-sm text-slate-950 sm:col-span-2">
                                        <textarea name="description" rows="2" class="rounded-xl bg-white px-3 py-2 text-sm text-slate-950 sm:col-span-2">{{ old('description', $task->description) }}</textarea>
                        ...
                        @unless ($task->isCompleted())
                            <div
                                ...
                                class="fixed inset-0 z-[60] hidden items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm"
